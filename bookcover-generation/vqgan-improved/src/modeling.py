...
class VQVAELayer(nn.Module):
    ...
    def forward(self, hidden: torch.Tensor) -> torch.Tensor:
        ...
        hidden = self.conv1(F.leaky_relu(hidden))
        hidden = self.conv2(self.upsample(F.leaky_relu(hidden)))
        hidden = self.conv3(self.pool(F.leaky_relu(hidden)))
...
class VQVAEEncoder(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        hidden = self.head(F.leaky_relu(hidden))
        return hidden
