...
class VQGANTrainingModule(LightningModule):
    ...
    def calculate_adaptive_weight(
        self, loss_content, loss_generator: torch.Tensor
    ) -> torch.Tensor:
        weight = self.decoder.head.weight
        content_grad = torch.autograd.grad(loss_content, weight, retain_graph=True)
        generator_grad = torch.autograd.grad(loss_generator, weight, retain_graph=True)

        adaptive_weight = content_grad[0].norm() / (generator_grad[0].norm() + 1e-4)
        return adaptive_weight.clamp(0, 1e4).detach()
    ...
    def generator_step(
        ...
        loss = (
            ...
            + self.generator_weight * adaptive_weight * loss_generator
        )
        metrics = {
    ...
    def configure_optimizers(
        ...
        generator_optimizer = Adam(generator_params, **self.config.optim.generator)
        discriminator_optimizer = Adam(
            discriminator_params, **self.config.optim.discriminator
        )
        ...
        generator_optimizer = {
            ...
            "scheduler": CosineAnnealingLR(
                generator_optimizer, self.config.train.epochs
            ),
        }
        discriminator_optimizer = {
            ...
            "scheduler": CosineAnnealingLR(
                discriminator_optimizer, self.config.train.epochs
            ),
        }
